...
void EuclideanClusterNodelet::onInit()
{
    ...
    nh_ = getNodeHandle();
    pointcloud_sub_ = nh_.subscribe("vscan/pointcloud", 1, &EuclideanClusterNodelet::pointcloudCallback, this);

    cluster_pub_ = nh_.advertise<autoware_msgs::DynamicObjectWithFeatureArray>("clusters", 10);
    debug_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("debug/clusters", 1);
}
